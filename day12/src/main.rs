...
fn main() {
    ...
    .expect("parse regex");
    let input: Vec<Instruction> = fs::read_to_string("day12/resources/input.txt")
...
fn run(instructions: &Vec<Instruction>, init_c: bool) {
    let mut instruction_pointer = 0_usize;
    let mut registers: HashMap<char, i32> = HashMap::from([('a', 0), ('b', 0), ('c', 0), ('d', 0)]);

    if init_c {
        *registers.get_mut(&'c').expect("register") = 1;
    }
    ...
    println!("a: {}", registers.get(&'a').expect("register"))
}
